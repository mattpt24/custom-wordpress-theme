...
*** To start up this development enviroment run - npm run watch-prod ***
...
1. npm init or npm init -y (Creates package.json if not included)
2. npm i
3. npm update // npm audit fix --force (To address all issues,including breaking changes)
...
7. npm install typescript ts-loader --save-dev
...
* npm run watch-prod   - Watches and compiles TS & SCSS files automatically (MINIFIED) USE THIS
* npx mix watch        - Watches and compiles JS & SCSS files automatically (NOT MINIFIED)
...
* Creating new .js and .scss files: 
   - When creating .scss files make sure to import in the style.scss file    (eg. @use "./components/buttons";)
...
ACF CUSTOM FIELDS                                  ------   <?php echo get_field('name_of_custom_field')?>

ACF CUSTOM FIELDS FROM ANOTHER PAGE                ------   <?php echo get_field('name_of_custom_field', 20)?>
...
<!-- DISPLAY THE VALUE OF A CUSTOM TAXONOMY INSIDE A WORDPRESS LOOP -->
...
<!-- $terms = get_the_terms( get_the_ID(), 'your-taxonomy-slug' );
...
DYNAMIC FILTERING WITH CUSTOM TAXONOMY (EXAMPLE IS PROJECT CATEGORY)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 <!-- FILTRATION  -->
  <div class="project-category-filtration pb-4" role="group" aria-label="Project Categories">
    <button class="btn is-active project-category-filter-btn" type="button" aria-controls="all-project-blocks" aria-selected="true" data-term-slug="all">All</button>
    <?php
    ...
    if (!empty($categories) && !is_wp_error($categories)) {
      foreach ($categories as $category) {
        echo '<button class="btn project-category-filter-btn" type="button" role="tab"" aria-controls="' . esc_attr($category->slug) . '-project-blocks" aria-selected="false" data-term-slug="' . esc_attr($category->slug) . '">' . esc_html($category->name) . '</button>';
      }
    }
    ?>
  </div>
    ...
    while($projects->have_posts()) {
        $projects->the_post();
            ...
            if (!empty($project_categories)) {
                $category_slug = $project_categories[0]->slug;
            }

        ?>

           <div class="project-block" data-category-slug="<?php echo esc_attr($category_slug); ?>">
